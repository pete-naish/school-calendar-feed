...
//
// A class's code ("y5-b") is a permanent slot; its label ("5HP") is what the
...
test("the title prefix is the label, whatever the code is", () => {
  ...
  assert.equal(titlePrefixFor("y5-b"), "5HP: ");
...
test("relabelling a class changes only its prefix", () => {
  ...
  try {
    ...
    assert.equal(titlePrefixFor("y5-a"), "5M: ");
...
test("a class's school events have the label prefix taken off (not the code's)", async () => {
  ...
  globalThis.fetch = async (url) => {
    ...
    return new Response(ics("5HP: Sports Day"), { status: 200 });
  };
...
test("the weekly list strips the label prefix, so a code-shaped prefix would be left in", () => {
  ...
  assert.match(week("5HP: Sports Day"), /• Sports Day/);
  assert.doesNotMatch(week("5HP: Sports Day"), /5HP/);
